...
// Contact form — send message directly to ekemini456@gmail.com
...
if (contactForm) {
  contactForm.addEventListener('submit', async e => {
    ...
    // Web3Forms Access Key for ekemini456@gmail.com
    // (Replace 'YOUR_WEB3FORMS_ACCESS_KEY' with your free key from https://web3forms.com)
      ...
      if (WEB3FORMS_KEY && WEB3FORMS_KEY !== 'YOUR_WEB3FORMS_ACCESS_KEY') {
        const res = await fetch('https://api.web3forms.com/submit', {
          ...
          body: JSON.stringify({
            ...
            email: email,
            service: service === 'Other' && other ? `Other - ${other}` : service,
            message: message,
            subject: `New Portfolio Inquiry from ${fname} ${lname}`
          })
        });
        const data = await res.json();
        if (data.success) {
          showSuccessMessage(fname, false);
          return;
        }
      }
    } catch (err) {
    ...
    // Fallback: Open mailto link pre-filled
    ...
    showSuccessMessage(fname, true);
  });
}

function showSuccessMessage(fname, isMailto = false) {
  const formContainer = document.querySelector('.contact-form');
  const text = isMailto
    ? `Your email app is opening with your message pre-filled to <strong>ekemini456@gmail.com</strong> — just hit send!`
    : `Thank you ${fname}, your message has been sent directly to <strong>ekemini456@gmail.com</strong>. I typically respond within 24 hours.`;

  formContainer.innerHTML = `
    <div style="background: rgba(0, 229, 160, 0.08); border: 1px solid var(--green); border-radius: 16px; padding: 2.5rem 1.8rem; text-align: center;">
      <i class="fa-solid fa-circle-check" style="font-size: 3rem; color: var(--green); margin-bottom: 1rem; display: block;"></i>
      <h3 style="font-size: 1.4rem; color: var(--white); margin-bottom: 0.6rem; font-weight: 700;">Message Sent!</h3>
      <p style="color: var(--muted); font-size: 0.95rem; line-height: 1.6;">${text}</p>
    </div>
  `;
}
